...
/* integrate smartPA cal function start */
...
#endif
#endif
/* integrate smartPA cal function end */
		...
		union afe_spkr_prot_config *prot_config, uint32_t param_size)
{
	...
	case AFE_PARAM_ID_SP_V4_EX_VI_FTM_CFG:
		...
		break;
/* integrate smartPA cal functionstart */
#ifdef CONFIG_SND_SOC_TFA98XX
...
int send_tfa_cal_apr(void *buf, int cmd_size, bool bRead)
{
	...
	if (0 != tfa_cal->map_data.map_handle) {
		...
		pr_err("%s: afe_unmap_rtac_block result %d\n", __func__, result);
	}
