...
#define CHIP_SELECTOR_LEFT_2	(3)
#define CHIP_SELECTOR_RIGHT_2	(4)
#define CHIP_SELECTOR_NONE	(5)
#define CHIP_SELECTOR_TOP	(6)
#define CHIP_SELECTOR_DOWN	(7)
#define CHIP_LEFT_ADDR		(0x34)
#define CHIP_RIGHT_ADDR		(0x35)
#define CHIP_LEFT_2_ADDR		(0x36)
    ...
    list_for_each_entry(tfa98xx, &tfa98xx_device_list, list) {
		...
		if (selector == CHIP_SELECTOR_LEFT) {
	        ...
	        if (tfa98xx->i2c->addr == CHIP_RIGHT_ADDR)
                ...
                else if (selector == CHIP_SELECTOR_TOP) {
                if (tfa98xx->i2c->addr == CHIP_RIGHT_ADDR | tfa98xx->i2c->addr == CHIP_LEFT_ADDR)
                    tfa98xx->flags |= TFA98XX_FLAG_CHIP_SELECTED;
                else
                    tfa98xx->flags &= ~TFA98XX_FLAG_CHIP_SELECTED;
                }

		else if (selector == CHIP_SELECTOR_LEFT_2) {
			if (tfa98xx->i2c->addr == CHIP_LEFT_2_ADDR)
				tfa98xx->flags |= TFA98XX_FLAG_CHIP_SELECTED;
			else
				tfa98xx->flags &= ~TFA98XX_FLAG_CHIP_SELECTED;
		}
	        ...
	        if (tfa98xx->i2c->addr == CHIP_RIGHT_2_ADDR)
                ...
                else if (selector == CHIP_SELECTOR_DOWN) {
                if (tfa98xx->i2c->addr == CHIP_RIGHT_2_ADDR | tfa98xx->i2c->addr == CHIP_LEFT_2_ADDR)
                    tfa98xx->flags |= TFA98XX_FLAG_CHIP_SELECTED;
                else
                    tfa98xx->flags &= ~TFA98XX_FLAG_CHIP_SELECTED;
                }
...
*/ // ZTE deleted for DMIC register in K65
