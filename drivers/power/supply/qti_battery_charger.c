...
struct battery_chg_dev {
	...
	/* extcon notifier for USB1 */
	struct extcon_nb		*extcon_nb;
	enum bcdev_id_state		id_state;
	struct work_struct		extcon_notify_work;
	u32				*thermal_levels;
					 ...
					 u32 adap_type)
{
	...
	pr_info("%s USB_SCOPE=%d\n", __func__, pst->prop[USB_SCOPE]);
		...
		union power_supply_propval *pval)
{
	...
	if (strstr(current_pid_name, "health") != NULL) {
...
#ifdef ZTE_CHARGER_EXT_BATTERY
	if (&bcdev->update_soc_work != NULL && bcdev->screen_is_on) {
		cancel_delayed_work(&bcdev->update_soc_work);
		schedule_delayed_work(&bcdev->update_soc_work, msecs_to_jiffies(50));
		pr_debug("start battery_chg_update_soc_work for screen on\n");
	}
#endif
	return count;
}
...
static void battery_chg_update_soc_work(struct work_struct *work)
{
	...
	if (&bcdev->update_soc_work != NULL) {
		...
		pr_debug("screen_is_on=%d,delay=%d\n", bcdev->screen_is_on, delay_ms);
...
static void battery_chg_extcon_notify_work(struct work_struct *work)
{
	struct battery_chg_dev *bcdev = container_of(work,
					struct battery_chg_dev, extcon_notify_work);
	int rc;

	// notify adsp here
	rc = write_property_id(bcdev, &bcdev->psy_list[PSY_TYPE_WLS],
				WLS_BOOST_EN, !bcdev->id_state);
}

static int battery_chg_id_notifier(struct notifier_block *nb,
	unsigned long event, void *ptr)
{
	struct extcon_dev *edev = ptr;
	struct extcon_nb *enb = container_of(nb, struct extcon_nb, id_nb);
	struct battery_chg_dev *bcdev = enb->bcdev;
	enum bcdev_id_state id;

	if (!edev || !bcdev)
		return NOTIFY_DONE;

	id = event ? BCDEV_ID_GROUND : BCDEV_ID_FLOAT;

	if (bcdev->id_state == id)
		return NOTIFY_DONE;

	// bcdev->ext_idx = enb->idx;

	dev_info(bcdev->dev, "extcon host:%ld (id:%d) event received\n", event, id);

	bcdev->id_state = id;

	schedule_work(&bcdev->extcon_notify_work);

	return NOTIFY_DONE;
}

static int battery_chg_extcon_register_notifier(struct battery_chg_dev *bcdev)
{
	struct device_node *node = bcdev->dev->of_node;
	struct extcon_dev *edev;
	...
	dev_info(bcdev->dev, "%s extcon_cnt:%d\n", __func__, extcon_cnt);
	if (extcon_cnt < 0) {
		dev_err(bcdev->dev, "of_count_phandle_with_args failed\n");
		return -ENODEV;
	}

	bcdev->extcon_nb = devm_kcalloc(bcdev->dev, extcon_cnt,
					sizeof(*bcdev->extcon_nb), GFP_KERNEL);
	if (!bcdev->extcon_nb)
		return -ENOMEM;

	for (idx = 0; idx < extcon_cnt; idx++) {
		edev = extcon_get_edev_by_phandle(bcdev->dev, idx);
		...
		bcdev->extcon_nb[idx].bcdev = bcdev;
		bcdev->extcon_nb[idx].edev = edev;
		bcdev->extcon_nb[idx].idx = idx;
		bcdev->extcon_nb[idx].id_nb.notifier_call = battery_chg_id_notifier;
		ret = extcon_register_notifier(edev, EXTCON_USB_HOST,
						&bcdev->extcon_nb[idx].id_nb);
		if (ret < 0)
			check_id_state = false;

		/* Update initial ID state */
		if (check_id_state && extcon_get_state(edev, EXTCON_USB_HOST)) {
			bcdev->id_state = BCDEV_ID_GROUND;
			battery_chg_id_notifier(&bcdev->extcon_nb[idx].id_nb,
						true, edev);
		} else {
			bcdev->id_state = BCDEV_ID_FLOAT;
		}
		dev_info(bcdev->dev, "%s init id_state:%d\n", __func__, bcdev->id_state);
	}

	if (!phandle_found) {
		pr_err("no extcon device found\n");
		return -ENODEV;
	}
...
static int battery_chg_probe(struct platform_device *pdev)
{
	...
	rc = battery_chg_extcon_register_notifier(bcdev);
	if (rc < 0)
		dev_err(dev, "Failed to register extcon notifier rc=%d\n", rc);
	else
		INIT_WORK(&bcdev->extcon_notify_work, battery_chg_extcon_notify_work);
